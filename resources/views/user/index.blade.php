            ...
            <div class="max-w-4xl mx-auto mt-6 mb-6">
                <form method="GET" action="{{ route('user') }}" class="flex flex-wrap gap-2 items-center">
                    ...
                    <select name="category" onchange="this.form.submit()" class="border rounded px-3 py-2">
                        <option value="gaming"
                            {{ request('category', $category ?? 'gaming') == 'gaming' ? 'selected' : '' }}>Gaming</option>
                        <option value="desain" {{ request('category', $category ?? '') == 'desain' ? 'selected' : '' }}>
                            Desain</option>
                        <option value="school" {{ request('category', $category ?? '') == 'school' ? 'selected' : '' }}>
                            School</option>
                        <option value="office" {{ request('category', $category ?? '') == 'office' ? 'selected' : '' }}>
                            Office</option>
                    </select>
                </form>
            </div>
                            ...
                            <div class="bg-white shadow rounded-lg p-4 flex flex-col h-full">
                                ...
                                <form method="POST" action="{{ route('laptop.compare.add') }}" class="mt-2">
                                    ...
                                    <button type="submit"
                                        ...
                                        <span class="block">Tambah ke Bandingkan</span>
                                    </button>
                                </form>
                            </div>
                        </a>
